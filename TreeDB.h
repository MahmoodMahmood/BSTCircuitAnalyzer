...
private:
   ...
   void recursiveInsert(TreeNode* current, TreeNode* newNode);
   DBentry* recursiveFind(TreeNode* current, string name);
   void recursiveLoadStream(TreeNode* current, ostream& out) const;
   TreeNode* recursiveGetNode(DBentry* entry, TreeNode* currentNode);
   TreeNode* getMaxNode(TreeNode* currentNode);
   TreeNode* recursiveFindParent(TreeNode* current, TreeNode* child);
   int recursiveCountActive(TreeNode* current, int total) const;
public:
